# TaxLossApp — Deployment Plan (localhost → production)

Companion to `PLAN.md`. That file covers *what we're building*; this one covers *how it goes
live*. It's intentionally forward-looking — we tackle the details when we reach M7
...
1. **Authentication exists** (PLAN §5 / M7). Today there is no auth and `BIND_ADDR` is
...
- **Frontend (M6):** `npm run build` → static assets served from the platform's CDN/static
...
- **Webhooks need a public HTTPS endpoint** and **signature verification** (PLAN §5, M2 item) —
...
- **CORS** (PLAN §5, M6): lock `tower-http` CORS to the exact frontend origin — no wildcards
...
## 11. Pre-launch security checklist (cross-ref PLAN §5)
